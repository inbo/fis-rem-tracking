...
table(gmu8$data$deployment$serie, 
      gmu8$data$deployment$year, useNA = "ifany")
table(gmu8$data$observations$scientificName, useNA = "ifany")
```
...
gmu8 <- gmu8 |> 
  filter_deployments(deploymentID %in% calibration_deps$deploymentID) |> 
  filter_observations(is.na(cameraSetupType))
```
...
seqID <- unique(gmu8$data$observations$eventID) # All seq that have to be done at some point in time
```
...
if(length(non_tracked_done) > 0){
  cat(length(non_tracked_done), "Non-tracked sequences detected on the sequences done list => asking rerun")
